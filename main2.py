...
def preprocess_packet(packet):
    features = {
        ...
        'duration': ...  # Extract other features
    }
    return features

def preprocess_realtime_data(packet):
    # Convert packet data to a DataFrame for preprocessing
    df = pd.DataFrame([preprocess_packet(packet)])
    
    # Perform one-hot encoding for categorical features
    encoded_data = pd.get_dummies(df[features_to_encode], drop_first=True)
    
    # Join with numeric features
    final_data = encoded_data.join(df[numeric_features])
...
def process_packet(packet):
    ...
    processed_data = preprocess_realtime_data(packet)
    
    # Predict using the trained model
    prediction = clf.predict(processed_data)
    
    # Update metrics based on the actual label (ground truth)
    actual_label = 1 if packet_has_malicious_activity(packet) else 0  # Adjust this based on your dataset
    ...
    cumulative_accuracy = accuracy_score([actual_label], [prediction])
    cumulative_precision = precision_score([actual_label], [prediction])
    cumulative_recall = recall_score([actual_label], [prediction])
    cumulative_f1 = f1_score([actual_label], [prediction])
...
def packet_has_malicious_activity(packet):
    # Adjust this function based on your dataset
    return True if packet[IP].proto == 1 else False
