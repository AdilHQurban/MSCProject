...
def packet_callback(packet):
    # Extract relevant information from the live packet
    # Update this function based on the features you want to capture
    src_ip = packet[IP].src
    dst_ip = packet[IP].dst
    protocol = packet[IP].proto  # Assuming IP layer is present

    # Perform real-time analysis (replace with your analysis logic)
    prediction = perform_real_time_analysis(packet)
...
def perform_real_time_analysis(packet):
    import random
...
def update_plot():
    plt.clf()
    plt.plot(time_points, prediction_points, marker='o', linestyle='-', color='b')
    plt.xlabel('Time')
    plt.ylabel('Prediction')
    plt.title('Real-time IDS Analysis')
    plt.grid(True)
    plt.pause(0.1)  


ani = FuncAnimation(plt.gcf(), packet_callback, interval=1000)

sniff(prn=lambda x: None, store=0)  
